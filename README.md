...
在您的仓库中，找到 sources/ 目录。您可以编辑现有文件，或创建一个新的分类文件（如 日韩频道.txt）。

在文件中按照 频道名称,直播链接 的格式添加新的直播源，每行一个。例如：

某高清电影频道,http://example.com/live/1.m3u8
某体育频道,http://example.com/live/2.m3u8

完成添加后，回到您的仓库主页，点击 "Contribute" -> "Open pull request"，向我们提交您的更改。
...
为您的新功能或修复创建一个新的分支 (git checkout -b feature/AmazingFeature)。

提交您的代码 (git commit -m 'Add some AmazingFeature')。

将您的分支推送到GitHub (git push origin feature/AmazingFeature)。

向我们提交一个 Pull Request，并详细说明您的改动。
